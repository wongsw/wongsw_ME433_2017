...
void APP_Initialize(void) {
    ...
    TRISAbits.TRISA1 = 0;
    LATAbits.LATA1 = 0; // A1 is the direction pin to go along with OC1 (values were chosen based on how it spun when attached to chassis)

    RPB2Rbits.RPB2R = 0b0101; // B2 is OC4
    TRISBbits.TRISB3 = 0;
    LATBbits.LATB3 = 1; // B3 is the direction pin to go along with OC4 (values were chosen based on how it spun when attached to chassis)
    ...
    switch (appData.state) {
        ...
        case APP_STATE_SCHEDULE_READ:
            ...
            if (appData.isReadComplete == true) {
                ...
                if (gotRx == 1) { // ensure that the wheels spin only when data is read (and don't start spinning everytime after it is programmed)
                    error = rxVal - 320; // 320 means the dot is in the middle of the screen (640 is the total width of screen)
                    ...
                    LATAbits.LATA1 = 0; // always go forward (values were chosen based on how it spun when attached to chassis) 
                    LATBbits.LATB3 = 1; // always go forward (values were chosen based on how it spun when attached to chassis)
                    OC1RS = left;
                    OC4RS = right;
                }
